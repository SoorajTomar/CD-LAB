...
void parse(char* str) //parse the expression
{
    ...
    while (right <= len && left <= right) {
        ...
        if (isPunctuator(str[right]) && left == right) //if character is a punctuator
            {
                if(isOperator(str[right])&& isOperator(str[right+1])){
                    std::cout<< str[right] <<str[right+1]<<" IS AN OPERATOR\n";
                    right++;
                }
